...
#MODIFY
    ...
    def build_subtree(self,node_id,subtree_node_id,depth, max_percent_leaf=0.3):
        if int(node_id) == 0:
            level = 0
            curr_node = 0
            nodes_to_check = 1
...
#dataset = np.loadtxt(IN_FILE, dtype=float,delimiter=',')
...
NE_RANGE = [100]
TD_RANGE = [10, 15, 20, 25, 30,35]

PL_RANGE = [0.3, 0.5, 0.7]
configs = []
...
for conf in configs:
    ...
    for _subtree_depth in range(9,10): 
        for mpl in PL_RANGE:
            ...
            treename = "clustered_variable_"+DATASET_NAME+"_td"+str(_max_depth)+"_ne"+str(_n_estimators) + "_sd" + str(_subtree_depth) + "_pl"+str(mpl)
