...
#MODIFY
    ...
    def build_subtree(self,node_id,subtree_node_id,depth, max_percent_leaf=0.3,use_pl=False, init_st_size=0):
        if int(node_id) == 0 and use_pl:
            level = 0
            curr_node = 0
            nodes_to_check = 1
            depths = [0]
        ...
        if int(node_id)==0 and init_st_size!=0:
            self.subtree_max_depth = init_st_size
            self.depth_map = [0]
            for i in range(0, init_st_size):
                res = self.depth_map[i] + int(math.pow(2.0, float(i)))
                self.depth_map.append(res)
            self.subtree_node_list = np.zeros(3*int(math.pow(2.0, float(init_st_size+1))))
...
dataset = np.loadtxt(IN_FILE, dtype=np.float32,delimiter=',')

#nrows_use = int((np.shape(dataset)[0])/4)
nrows_use = _n_samples
X_all = dataset[0:nrows_use,1:]
y_all = dataset[0:nrows_use, 0]
...
for conf in configs:
    ...
    treename = "clustered_var_"+DATASET_NAME+"_td"+str(_max_depth)+"_ne"+str(_n_estimators) 
    ...
    for _subtree_depth in SD_RANGE: 
        for inst in INIT_RANGE:
            forest_trees = []
            #_subtree_depth = 2 
            ...
            #    csr_decision_tree = [num_of_nodes, node_list, edge_list, node_is_leaf, node_features, node_values]
                ...
                while len(pending_subtrees_to_build) > 0:
                    ...
                    subtree.build_subtree(node_id=ori_node_id,subtree_node_id=0,depth=0, max_percent_leaf=0.3, init_st_size=inst)
                    subtree.build_subtree_idx_to_other_subtree()
                    
                    decision_tree[sub_tree_num]=subtree
                
                print("Done transforming tree "+str(i))
            ...
            treename = "clustered_var_"+DATASET_NAME+"_td"+str(_max_depth)+"_ne"+str(_n_estimators) + "_sd" + str(_subtree_depth)+"_istd"+str(inst)
