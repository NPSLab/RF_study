...
hier_kernel(
  ...
  unsigned num_of_subtrees = prefix_sum_subtree_nums[tree_num+1]-tree_offset;

  int *curr_st_traverse = &d_curr_st_traverse[tree_num*num_of_queries];

  int nodes_per_subtree = pow(2.0,max_st_depth) - 1;
  int max_subtrees_loadable = (RATIO_ST*SM_BYTES)/(3*sizeof(float)*nodes_per_subtree);

  int global_subtree_idx = 0;
  while (global_subtree_idx < num_of_subtrees)
  {
    ...
    for (int st = global_subtree_idx; st < global_subtree_idx+subtrees_loaded; st++)
    {
      for (int tid = threadIdx.x + queries_start; tid < queries_end; tid += blockDim.x)
      {
        float * row = queries + tid*num_of_features; 
        if (st == curr_st_traverse[tid])
        {
          
          const unsigned subtree_leaf_idx_boundry = leaf_idx_boundry[tree_offset+st];         // Fetch leaf boundary of current subtree
      
          const unsigned *subtree_idx_to_subtree = idx_to_subtree + g_subtree_idx_to_subtree_offset[tree_offset+st]*2;
          ...
          while (true){
            ...
            // if reach bottom of subtree, then we need to go to another subtree
                ...
                if (go_left){
                    curr_st_traverse[tid] = subtree_idx_to_subtree[2*leaf_idx];
                }else{
                    curr_st_traverse[tid] = subtree_idx_to_subtree[2*leaf_idx+1];
                }    
...
//       vector<unsigned> g_subtree_idx_to_subtree_offset   ;
  ...
  cudaMalloc((void**)&d_curr_st_traverse                     ,sizeof( int      )*num_of_trees*(row/QUERIES_PASSES));
  ...
  START_TIMER
  for (int pass = 0; pass < QUERIES_PASSES; pass++)
  {
