...
hier_kernel(
  ...
  unsigned *results                  
    ...
    for (int blk = blockIdx.x; blk < num_of_queries; blk += gridDim.x){
      //fetch a new query
      float * row = queries + blk*num_of_features; 
         for(int tree_num = threadIdx.x; tree_num < num_of_trees; tree_num+=blockDim.x){
            //go over trees
            //tree_num=0;
...
//       vector<unsigned> g_subtree_idx_to_subtree_offset   ;
  ...
  generate_results<<<1000,64>>>(row, num_of_trees, d_results);
  cudaDeviceSynchronize();
