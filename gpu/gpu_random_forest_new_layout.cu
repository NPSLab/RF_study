      ...
      float * row = queries + tid*num_of_features; 
         for(int tree_num = 0; tree_num < num_of_trees; ++tree_num){
            ...
            while (true){
              ...
              //Mod
                const float *subtree_node_value_list;
                subtree_node_value_list = nodes_value + g_subtree_nodes_offset[tree_off_set+curr_subtree_idx];

                const unsigned short *subtree_node_is_leaf_feature_id_list;
                subtree_node_is_leaf_feature_id_list = nodes_is_leaf_feature_id + g_subtree_nodes_offset[tree_off_set+curr_subtree_idx];
                ...
                while (true){
                  //Mod
                    unsigned short feature_id = subtree_node_is_leaf_feature_id_list[curr_node]&(~(0x1<<15));
                    float node_value    = subtree_node_value_list[curr_node];
                    unsigned short is_tree_leaf    = subtree_node_is_leaf_feature_id_list[curr_node]>>15;
...
//We divide nodes array into two arrays to save memory 
  ...
  vector<unsigned short> nodes_is_leaf_feature_id(num_nodes); 
  vector<float>          nodes_value(num_nodes);
  for (unsigned i=0;i< num_nodes;++i){
                    nodes_is_leaf_feature_id[i] = nodes[i*3];
                    nodes_value[i]              = nodes[i*3+1];
                    unsigned is_tree_leaf       = nodes[i*3+2];
                    if (is_tree_leaf == 1){
                      nodes_is_leaf_feature_id[i] |= (0x1<<15);
                    }
  }
...
//Mod nodes layout
  cudaMalloc((void**)&d_nodes_value                          ,sizeof( float    )*num_nodes                                 );
  cudaMalloc((void**)&d_nodes_is_leaf_feature_id             ,sizeof( unsigned short    )*num_nodes                        );
...
//Mod nodes layout
  cudaMemcpy( d_nodes_value                         ,nodes_value.data()                          ,sizeof( float    )*num_nodes                          ,cudaMemcpyHostToDevice);
  cudaMemcpy( d_nodes_is_leaf_feature_id                          ,nodes_is_leaf_feature_id.data()                          ,sizeof( unsigned short    )*num_nodes                          ,cudaMemcpyHostToDevice);
