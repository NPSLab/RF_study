      ...
      float * row = queries + tid*num_of_features; 
         for(int tree_num = 0; tree_num < num_of_trees; ++tree_num){
            ...
            while (true){
                ...
                #include "./generate_encoded_topology/et.inc"
...
template <typename T>
void read_arr(ifstream &infile, vector<T> &output ,string var_name);
template <typename T>
...
int main(){
  ...
  read_arr(infile, node_list_idx          , "node_list_idx"         );
