...
void hier_kernel(
  ...
  // U250 URAM per SLR: 11.52 MB
    ...
    #endif
	// #pragma HLS DATAFLOW
    unsigned char query_results[MAX_BUFFER_SIZE]; // 0.375 MB
    // #pragma HLS bind_storage variable=query_results type=RAM_2P impl=bram
        TREE_ITERATION:
        for(int tree_num = 0; tree_num < num_of_trees; ++tree_num){
          #pragma HLS LOOP_TRIPCOUNT min = 100 max = 128 avg = 100
          unsigned query_curr_subtree[MAX_BUFFER_SIZE]; // 1.5 MB
          #pragma HLS bind_storage variable=query_curr_subtree type=RAM_2P impl=uram
          unsigned query_data_buf[MAX_BUFFER_SIZE]; // 1.5MB
          #pragma HLS bind_storage variable=query_data_buf type=RAM_2P impl=uram
          ...
          for(unsigned curr_subtree_idx = 0;
            ...
            //Mod
              ...
              for(int subtree_depth = 0; subtree_depth < current_depth; subtree_depth++){
                ...
                #pragma HLS DEPENDENCE variable=query_curr_subtree inter RAW false
				        #pragma HLS DEPENDENCE variable=query_data_buf inter RAW false
				        #pragma HLS DEPENDENCE variable=query_results inter RAW false
                #ifndef __SYNTHESIS__
                  //  if(tid == 0)
                ...
                query_entry = query_data_buf[tid];
                ...
                if(query_curr_subtree[tid] == curr_subtree_idx && !(query_entry & 0x1)){
                  ...
                  if (is_tree_leaf==1){
                    ...
                    query_data_buf[tid] = query_entry | 0x00000001;
                  }
                  else{
                    ...
                    if (not_subtree_bottom){
                        ...
                        query_data_buf[tid] = curr_node << 1;
