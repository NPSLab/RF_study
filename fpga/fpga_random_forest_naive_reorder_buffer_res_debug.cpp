...
void hier_kernel(
  ...
  unsigned * results                  
    ...
    for (int tid = 0; tid < num_of_queries; tid++){
      ...
      if(tid == 420442) printf("New Query tid=%d\n", tid);
      //fetch a new query
      ...
      for(int i = 0; i < num_of_features; i++){
        ...
        queries_buf[i] = queries[row_offset + i];
         ...
         for(int tree_num = 0; tree_num < num_of_trees; ++tree_num){
            ...
            if(tid == 420442) printf("tree_off_set = %d\n", tree_off_set);
            ...
            if(tid == 420442) printf("Subtree Iteration tid=%d\n", tid);
            if(tid == 420442) printf("Tree Num=%d\n", tree_num);

            SUBTREE_ITERATION:
            //iterate over subtree
            while (true){
              #pragma HLS LOOP_TRIPCOUNT min = 1 max = 2 avg = 2
              if(tid == 420442) printf("Subtree Index = %d\n", curr_subtree_idx);
                ...
                while (true){
                  ...
                  //Mod
                    ...
                    if (is_tree_leaf==1){ result_temp += node_value; return_from_curr_tree = true; //if(tid == 499){
                        if(tid == 420442) printf("Leaf! curr_node = %d, curr_subtree_idx = %d, tid = %d\n", curr_node, curr_subtree_idx, tid);
                      goto SUBTREE_END;
                    }
